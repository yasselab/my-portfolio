...
export default function About() {
  ...
  const t = { soft: "Soft Skills" };

  const angleRef = useRef(0);
  const [tick, setTick] = useState(0); // léger re-render (10–20 FPS) → fluide et stable
  ...
  useEffect(() => {
    ...
    const animate = (timestamp) => {
      if (timestamp - last > 16) {
        ...
        setTick((t) => t + 1); // rafraîchit positions
        last = timestamp;
      }
    ...
    >
      ...
      >
        ...
        <div className="hidden lg:flex justify-center">
          <div className="w-[2px] h-72 bg-gradient-to-b from-[#D9A86C] via-[#C89155] to-[#D9A86C] opacity-90 rounded-full" />
        </div>
        ...
        >
          ...
          <div className="absolute inset-y-10 w-[260px] rounded-full bg-white/40 dark:bg-black/30 blur-3xl pointer-events-none" />

          {/* WHEEL */}
          <div className="relative h-[260px] w-[260px]">
            {skills.map((label, i) => {
              const radius = 100;
              const offsetAngle = angleRef.current + i * (360 / skills.length);
              ...
              // normalize
              const normalized = (offsetAngle % 360 + 360) % 360;

              // 🔥 skill à DROITE entre 340° et 20°
              const isRight = normalized > 290 && normalized < 350;
              ...
              return (
                <div
                  ...
                  className={`
                    ...
                    ${
                      isRight
                        ? "h-28 w-28 text-[13px] font-semibold text-white bg-gradient-to-br from-[#D9A86C] to-[#C89155] shadow-xl scale-110"
                        : "h-16 w-16 text-[11px] bg-white dark:bg-darkbrand-bg/80 text-brand-text/70 dark:text-darkbrand-text/70 shadow-md opacity-90"
                    }
                  `}
                  style={{
                    transform: `
